<?php defined('_JEXEC') or die('Restricted access'); ?>
		...
		<tr>
            ...
            <?php echo JText::_( 'Define' )." ".JText::_( 'User' ); ?>
            <br />
                <span class="small">
                  <?php echo $this->user->id; ?>: <?php echo htmlspecialchars( $this->user->name ); ?> (<?php echo htmlspecialchars( $this->user->username ); ?>)<br /> &nbsp;&nbsp;&bull;&nbsp;&nbsp;<?php echo htmlspecialchars( $this->user->email ); ?>
                </span>            
            </th>            
        ...
        for ($i=0, $n=count( $this->items ); $i < $n; $i++)
        {
            ...
            $link = JRoute::_( 'index.php?option='.$option.'&controller=groups&task=edit&cid[]='. $r->id );
//            $link = JRoute::_( 'index.php?option='.$option.'&controller='.$this->_name.'&task=edit&cid[]='. $r->id );

			$database->setQuery("SELECT `user_id` FROM  " . TABLE_YOONIQUEACL_U2G
								." WHERE `group_id` = '$r->id'"
								." AND `user_id` = '".$this->user->id."' ");
			$already = $database->loadResult();

			$img_u 	= $already ? 'tick.png' : 'publish_x.png';
			$alt_u 	= $already ? JText::_( 'Member' ) : JText::_( 'None' );
		...
		<input type="hidden" name="id" value="<?php echo htmlspecialchars( $this->user->id ); ?>" />
