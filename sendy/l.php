...
<?php
	...
	if($ares_emails_id=='') //if link does not belong to an autoresponder campaign
	{
		...
		$link = str_replace('[unsubscribe]', APP_PATH.'/unsubscribe/'.short($email).'/'.short($userID).'/'.short($campaign_id), $link);
	}
	else
	{
		$link = str_replace('[webversion]', APP_PATH.'/w/'.short($userID).'/'.short($list_id).'/'.short($campaign_id).'/a', $link);
		$link = str_replace('[unsubscribe]', APP_PATH.'/unsubscribe/'.short($email).'/'.short($userID).'/'.short($campaign_id).'/a', $link);
	}
